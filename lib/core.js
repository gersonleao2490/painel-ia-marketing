...
function geminiGenerate(key, model, messages, options) {
      ...
      gres => {
        let body = ''; gres.on('data', c => body += c); gres.on('end', () => {
          try {
            ...
            if (gres.statusCode !== 200 || j.error) return resolve({ error: '⚠️ IA na nuvem recusou: ' + ((j.error && j.error.message) || ('HTTP ' + gres.statusCode)) + ' — confira a chave do Gemini.' });
          ...
          } catch (e) { resolve({ error: '⚠️ erro lendo a resposta da IA na nuvem.' }); }
        });
      });
    r.on('error', e => resolve({ error: '⚠️ não consegui falar com a IA na nuvem: ' + e.message }));
    r.write(data); r.end();
  });
}
...
function igFetchProfile(igCfg, handle) {
  return new Promise(resolve => {
    const host = igCfg.host || 'instagram-scraper-api2.p.rapidapi.com';
    const p = String(igCfg.path || '/v1/info?username_or_id_or_url={handle}').replace('{handle}', encodeURIComponent(handle));
    const r = https.request({ hostname: host, path: p, method: 'GET', headers: { 'x-rapidapi-key': igCfg.key, 'x-rapidapi-host': host } },
    ...
    r.on('error', () => resolve({ status: 0, json: null })); r.end();
  });
}
